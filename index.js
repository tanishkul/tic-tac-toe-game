...
var MYAPP = MYAPP || {
    ...
    initializeGame: function () {
        ...
        document.querySelector('.game-choice button').addEventListener("click", () => {
            ...
            document.querySelector('.game-starter .choose-x, .game-starter .choose-o').off().addEventListener("click", () => MYAPP.game.firstGame);
            var searchElement = document.createElement("div");
            document.querySelector('.back-button').appendChild(searchElement);
        ...
        document.querySelector('.hard-reset').on('click', MYAPP.game.resetGame);
    }
};
...
MYAPP.display = {
    hideGameStarter: function () {
        document.querySelector('.game-starter').fadeOut();
    },
    ...
    showGameStarter: function (isTwoPlayer) {
        ...
        MYAPP.timeOuts.push(
            setTimeout(function () {
                document.querySelector('.game-starter').fadeIn(500).children('p').text(message);
            }, 700));
    },

    showGameChoice: function () {
        document.querySelector('.game-choice').fadeIn(600);
    },

    hideGameChoice: function () {
        document.querySelector('.game-choice').fadeOut(600);
    },

    showPlayerOnePrompt: function () {
        if (MYAPP.secondPlayer) {
            document.querySelector('.player-one-turn p').text('Go Player 1!');
        } else {
            document.querySelector('.player-one-turn p').text('Your turn!');
        }
        document.querySelector('.player-one-turn').animate({
            'top': '-45px'
        }, 500);
    },
    ...
    showPlayerTwoPrompt: function () {
        if (MYAPP.secondPlayer) {
            document.querySelector('.player-two-turn p').text('Go Player 2!');
        } else {
            document.querySelector('.player-two-turn p').text('Computer\'s turn');
        }
        document.querySelector('.player-two-turn').animate({
            'top': '-45px'
        }, 500);
    },
    ...
    showDrawMessage: function () {
        MYAPP.timeOuts.push(
            setTimeout(function () {
                document.querySelector('.draw-message').fadeIn(500);
            }, 1500));
    },

    hideDrawMessage: function () {
        document.querySelector('.draw-message').fadeOut(1000);
    },

    showLoseMessage: function () {
        MYAPP.timeOuts.push(
            setTimeout(function () {
                document.querySelector('.lose-message').fadeIn(500);
            }, 1500)
        );
    },

    hideLoseMessage: function () {
        document.querySelector('.lose-message').fadeOut(1000);
    },

    showWinMessage: function () {
        MYAPP.timeOuts.push(
            setTimeout(function () {
                document.querySelector('.win-message').fadeIn(500).children('p').text("Player " + MYAPP.turn + " wins!! :D ")
            }, 1500));
    },

    hideWinMessage: function () {
        document.querySelector('.win-message').fadeOut(1000);
    },
    ...
    resetSquares: function () {
        document.querySelector('.boxes').html('');
        for (var i = 1; i <= 9; i++) {
            var box = '<li class="' + i + '"><i class="letter"><span></span></i></li>';
            document.querySelector(box).appendTo(document.querySelector('.boxes'));
        }
    },

    showScore: function () {
        if (MYAPP.secondPlayer) {
            document.querySelector('.score-1').children('.name').text('player 1');
            document.querySelector('.score-2').children('.name').text('player 2');
        } else {
            document.querySelector('.score-1').children('.name').text('player 1');
            document.querySelector('.score-2').children('.name').text('computer');
        }
        document.querySelector('.score-1, .score-2').children('.points').text('0');
        document.querySelector('.score-1,.score-2, .points-divider').fadeIn();
    },
    updateScore: function (turn) {
        var currentScore = turn === 1 ? MYAPP.playerOneScore : MYAPP.playerTwoScore;

        document.querySelector('.score-' + turn).children('.points').text(currentScore);
    }
};
...
MYAPP.game = {
    ...
    gameSelection: function (item) {
        if (document.querySelector(item).text() === 'One Player') {
    ...
    firstGame: function () {
        MYAPP.playerOneSymbol = document.querySelector(this).text();
        ...
        document.querySelector('.hard-reset').fadeIn(600);
    ...
    play: function () {
        ...
        document.querySelector('.boxes li').on('click', function () {
    ...
    playerTurn: function (square) {
        ...
        var box = document.querySelector(square).children('i').children('span');
        if (box.text() === '' && MYAPP.gameInPlay && (MYAPP.turn === 1 || (MYAPP.turn === 2 && MYAPP.secondPlayer))) {
            box.text(symbol);
            var number = document.querySelector(square).attr('class');
    ...
    computerPlay: function () {
        ...
        if (computer.computerWhichMove(MYAPP.game) && MYAPP.turn === 2) {
            ...
            var currentBox = document.querySelector('.' + boxNumber).children('i');
    ...
    showWinningCombination: function () {
        ...
        for (var i = 0; i < combo.length; i++) {
            ...
            document.querySelector(currentBox).children('i').addClass('win').children('span').addClass('rotate');
        }
    },
    updateScore: function (turn) {
    ...
    reset: function () {
        ...
        MYAPP.timeOuts.push(
            setTimeout(function () {
                ...
                document.querySelector('.boxes li').fadeOut();
            }, 5000),
            setTimeout(function () {
                MYAPP.display.resetSquares();
                document.querySelector('.boxes li').fadeIn();
                MYAPP.numFilledIn = 0;
            }, 6000),
    ...
    resetGame: function () {
        document.querySelector('#myCanvas').css('opacity', '0');
        document.querySelector('.hard-reset').fadeOut();
        document.querySelector('.points-divider, .score-1, .score-2').fadeOut();
        ...
        document.querySelectorAll('.draw-message, .win-message, .lose-message').forEach(box => { box.style.display = "none" })
        MYAPP.display.hidePlayerOnePrompt();
        MYAPP.display.hidePlayerTwoPrompt();
        MYAPP.display.showGameChoice();
    }
...
MYAPP.computer = {
    ...
    doubleThreatChoice: function (choiceType) {
        ...
        if (!move) {
            // clone current board;
            var testBoard = document.querySelector.extend({}, board);
            for (var i = 1; i <= 9; i++) {

                testBoard = document.querySelector.extend({}, board);
                if (testBoard[i] === '') {
